...
findOFBoundary <- function(corr, alpha, tol=0.000001, startvalue=0, lambda=0.01, iota=sqrt(2),
                           L=NULL){  # null L is bad. 
  # alpha needs to be between 0 and 1
  stopifnot(alpha<1, alpha>0)
  # start with confidence level of zero
  confLevel <- 0
  ...
  # check if start value is too high
  upp <- rep(bound, dim(corr)[1])  # initialize bound
  upp <- rep(c(iota, 1), each=L) * upp  # adjust for information; this is L dependent
  # iota will be proportion at first analysis, then second, etc...
  
  # upp <- c(iota*rep(bound, dim(corr)[1]/2), rep(bound, dim(corr)[1]/2))
...
# (value pattern repeated S times) * sqrt(c(n(s), n) ) / (sqrt(diag(V)))
...
findPocockSample <- function(corr, pow, cov, vp, bound, interim=0.5,
  ...
  S <- length(interim) + 1  # number of analyses is number of interim + 1
  L <- length(vp) / S
  ...
  # build mualt
  mualt <- as.vector(sqrt(rep(N, each=L) / cov) * vp)
