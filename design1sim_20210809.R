...
simulationOnce <- function(maxN, allN, bounds, regimes, vp, loopno, s2, H0,
  ...
  aa_p_fin <- (sum((((aares[['Pocock_Mid']]$value - H0) / aares[['Pocock_Mid']]$se) > 
                        aaBounds$Pocock$bound) > 0) > 0)*1
  # give results
  ...
  aa_p_fin <- (sum((((aares[['OF_Mid']]$value - H0) / aares[['OF_Mid']]$se) > 
  ...
  cd_p_fin <- (sum((((aares[['Pocock_Mid']]$value - H0) / cdres[['Pocock_Mid']]$se) > 
                      cdBounds$Pocock$bound) > 0) > 0)*1
  # give results
  ...
  cd_p_fin <- (sum((((aares[['OF_Mid']]$value - H0) / cdres[['OF_Mid']]$se) > 
...
if(substr(getwd(), 2, 5) == 'home'){
  main(varest=FALSE, sampleest=FALSE, boundest=FALSE, simyn=TRUE,
       unpacksim = TRUE, newdir=FALSE, prevdate="2021-08-10")
  #2021-08-10
}
